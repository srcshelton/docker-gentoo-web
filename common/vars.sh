...
if command -v cpuid2cpuflags >/dev/null 2>&1; then
	...
	description="$( grep 'model name' /proc/cpuinfo | sort | head -n 1 )"
	case "${description}" in
		...
		*)
			echo >&2 "Unknown CPU '$( echo "${desccription}" | cut -d':' -f 2- | sed 's/^\s*// ; s/\s*$//' )' - not enabling model-specific CPU flags" ;;
	esac
fi
